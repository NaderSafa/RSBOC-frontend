                ...
                <CardBody>
                  ...
                  <div className='wid-peity mb-4'>
                    <div className='row'>
                      {data?.topManufacturers?.map((item, idx) => (
                        <React.Fragment key={idx}>
                          <div>
                            ...
                            <p className='text-muted mb-4'>
                              Total: {formatCurrency(item?.totalExtendedPrice)}{' '}
                              | PA:
                      ...
                      <button
                        onClick={() => navigate('reports/manufacturers')}
                      ...
                      {data?.topMolecules?.map((item, idx) => (
                        <React.Fragment key={idx}>
                          <div
                            onClick={() =>
                              navigate(
                                `reports/molecules/${item?.drugIdentificationNo}`
                              )
                            }
                          >
                            <h5>
                            ...
                            <p className='text-muted mb-4'>
                              Total: {formatCurrency(item?.totalExtendedPrice)}{' '}
                              | PA: {formatCurrency(item?.totalRefundedAmount)}
                            </p>
                          </div>
                        </React.Fragment>
                      ))}
                      <button
                        onClick={() => navigate('reports/molecules')}
                        className='text-center'
                        style={{
                          backgroundColor: 'transparent',
                          border: '0',
                      ...
                      {data?.topPharmacies?.map((item, idx) => (
                        <React.Fragment key={idx}>
                          <div>
                            <h5>{item?.pharmacy}</h5>
                            <p className='text-muted mb-4'>
                              Total: {formatCurrency(item?.totalOrdersAmount)}
                            </p>
                          </div>
                        </React.Fragment>
                      ))}
